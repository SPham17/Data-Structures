...
class List:
    ...
    def _linear_search(self, key):
        ...
        current = None
        index = 0
        node = _List_Node(self._front, self._front._next)
        
        while node != key:
            previous = current
            current = current._next
            
            node = _List_Node(previous, current)
    ...
    def peek(self):
        ...
        value = _List_Node(self._front,self._front._next)
    ...
    def max(self):
        ...
        max_data = self._first
        
        while self._first is not None:
            
            if self._first > max_data:
                max_data = self._first
            
            else:
                self._first = _List_Node(self._first,)
